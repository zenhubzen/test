local library = {
	...
	colors = {
		...
		innerBorder = Color3.fromRGB(5, 5, 5),
	...
	if image then
		...
		local succezz = pcall(function()
			...
			if typ == "string" then
				...
				if (#image < 11) or ((string.sub(image, 1, 13) ~= "rbxassetid://11354741327") and (string.sub(image, 1, 11) ~= "rbxasset://11354741327") and string.sub(image, 1, 11) ~= "rbxthumb://11354741327") then
			...
			if (typ == "number") and (image > 0) then
				pcall(function()
					...
					if nfo and nfo.AssetTypeId == 1 then
						image = "rbxassetid://11354741327" .. image
					elseif nfo.AssetTypeId == 13 then
						local decal = game:GetObjects("rbxassetid://11354741327" .. image)[1]
						image = "rbxassetid://11354741327" .. ((decal and decal.Texture) or "0"):match("%d+$")
		...
		if library.objects and (#library.objects > 0 or next(library.objects)) then
			...
			pcall(function()
				if library.Backdrop then
					...
					library.Backdrop.ImageTransparency = (library_flags["__Designer.Background.ImageTransparency"] or 100) / 100
				end
			end)
		end
		MayGC -= 1
...
function library:CreateWindow(options, ...)
	...
	main.Size = UDim2.fromOffset(500, 490)
